...
<!-- DETAILS -->
1. Pre-Trained Model
The code leverages MediaPipe Hands:

MediaPipe Hands is a pre-trained model provided by Google that performs hand landmark detection and tracks multiple hands in real-time.
It outputs 21 key points per hand, representing significant landmarks like fingertips, knuckles, and the wrist.
Algorithm in MediaPipe:

MediaPipe uses a combination of machine learning-based detection and pose estimation algorithms.
The model detects hands in the image and tracks landmarks using regression-based pose estimation techniques.
2. Custom Models
The script integrates two custom models:

a. KeyPointClassifier
Purpose: Classifies the detected hand keypoints into specific hand signs (e.g., gestures for numbers or actions).
Input: Pre-processed, normalized 21 hand landmarks.
Algorithm: Likely uses a shallow neural network (DNN) for classification.
Training Data: Labeled data from the CSV (keypoint_classifier_label.csv).
b. PointHistoryClassifier
Purpose: Recognizes gestures based on the temporal movement of the hand (e.g., swipe gestures).
Input: History of tracked fingertip points (point_history).
Algorithm: Likely employs a recurrent neural network (RNN) or a simpler approach like a decision tree to classify sequences.
Training Data: Labeled data from the CSV (point_history_classifier_label.csv).
3. Machine Learning Algorithms
From the context of the script, these are the likely ML algorithms:

MediaPipe Hands:
Pose estimation via regression: Models trained on annotated hand data for robust landmark detection.
KeyPointClassifier:
Multiclass Classification Algorithm: Likely a shallow neural network or a decision tree-based model (based on the simplicity of the task).
PointHistoryClassifier:
Sequence-based Model: Could be:
RNN or LSTM: Handles time-series data (like gesture sequences).
Rule-based classifier: Simpler heuristics for shorter gesture patterns.
Workflow Summary
MediaPipe Hands extracts 21 landmarks from the hand.
These landmarks are normalized and fed into:
KeyPointClassifier: For static hand gesture classification.
PointHistoryClassifier: For dynamic gesture recognition based on movement history.
The results are visualized with OpenCV.